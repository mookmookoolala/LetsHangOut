...
export function Chat({ user, group }) {
  ...
  // Scroll to bottom on new messages (only chat area)
  useEffect(() => {
  ...
  return (
    <Card elevation={3} sx={{
      ...
      <CardHeader
        title={<Typography variant="h6" sx={{ fontWeight: 700, color: '#1976d2' }}>Group Chat</Typography>}
        sx={{ pb: 0, pt: 2, px: 3, bgcolor: 'transparent' }}
      />
      <CardContent sx={{ flex: 1, display: 'flex', flexDirection: 'column', p: isMobile ? 1.5 : 2, pt: 1 }}>
        <Box
          sx={{
            flex: 1,
        ...
        >
          {(messages || []).map((msg, idx) => (
            <Box key={msg.id || idx} sx={{
              ...
              boxShadow: msg.username === user?.username ? 1 : 0,
              maxWidth: '100%',
            }}>
              <Avatar sx={{ width: 28, height: 28, fontSize: 14, bgcolor: '#1976d2', mt: 0.5 }}>
                {msg.username ? msg.username[0].toUpperCase() : '?'}
              </Avatar>
              <Box sx={{ flex: 1, minWidth: 0 }}>
                <Typography variant="caption" sx={{ fontWeight: 600, color: '#1976d2', mb: 0.5, wordBreak: 'break-all' }}>
                  {msg.username}
                </Typography>
        ...
        <Box sx={{ display: 'flex', gap: 1, alignItems: 'center', mt: 1 }}>
          <TextField
            ...
            inputProps={{ style: { padding: isMobile ? '14px 12px' : '10px 12px' } }}
          />
          <Button
            variant="contained"
            onClick={handleSend}
